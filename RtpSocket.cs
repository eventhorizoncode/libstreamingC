...
namespace Net.Majorkernelpanic.Streaming
{
     ...
     */
    public class RtpSocket : IRunnable{

	    public const System.String TAG = "RtpSocket";

	    /** Use this to use UDP for the transport protocol. */
	    public const int TRANSPORT_UDP = 0x00;
	
	    /** Use this to use TCP for the transport protocol. */
	    public const int TRANSPORT_TCP = 0x01;	
	
	    public const int RTP_HEADER_LENGTH = 12;
	    ...
	    private Java.Util.Concurrent.Semaphore mBufferRequested, mBufferCommitted;
	    ...
	    private void resetFifo() {
		    ...
		    mBufferRequested = new Java.Util.Concurrent.Semaphore(mBufferCount);
		    mBufferCommitted = new Java.Util.Concurrent.Semaphore(0);
		    mReport.R();
	    ...
	    public void setDestination(InetAddress dest, int dport, int rtcpPort) {
		    if (dport != 0 && rtcpPort != 0) {
			    ...
			    mPort = dport;
			    for (int i=0;i<mBufferCount;i++) {
				    mPackets[i].Port = dport;
				    mPackets[i].Address = dest;
			    }
			    ...
			    while (mBufferCommitted.TryAcquire(4,TimeUnit.Seconds)) {
				    if (mOldTimestamp != 0) {
					    ...
					    if ((mTimestamps[mBufferOut]-mOldTimestamp)>0) {
						    ...
						    if (mCacheSize>0) Java.Lang.Thread.Sleep(d);
	    ...
	    private void sendTCP() {
		    lock (mOutputStream) {
			    int len = mPackets[mBufferOut].Length;
			    Log.d(TAG,"sent "+len);
        ...
        public class AverageBitrate {
		    ...
		    private const long RESOLUTION = 200;
